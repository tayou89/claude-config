...
코드를 작성하거나 수정할 때 **업계 표준/베스트 프랙티스**를 따른다. 기존 코드가 표준과 다른 방식으로 되어 있을 경우:

1. 사용자에게 **어떤 부분이 표준과 다른지** 구체적으로 알린다
2. **표준 방식과 현재 방식**을 비교하여 설명한다
3. 어떤 방식으로 진행할지 **사용자에게 확인**받은 후 진행한다

표준을 모를 경우 임의로 판단하지 않고, 일반적으로 널리 쓰이는 패턴(TypeScript 공식 문서, 주요 프레임워크 컨벤션 등)을 기준으로 한다.
...
문제를 만나면 시간이 걸리더라도 **근본 원인을 먼저 파악하고 해결**한다. **코드 에러, 타입 에러, 런타임 오류 모두 동일하게 적용한다.** 타입 에러가 발생해도 캐스팅이나 인덱스 시그니처 추가로 회피하지 않고 원인을 찾는다.

1. 증상이 아닌 **원인**을 찾는다
   - 런타임: "A가 동작하지 않는다" → "왜 안 되는가?" → 실제 원인
   - 타입: "`FrameParam`이 `Record<string, unknown>`에 호환 안 됨" → "왜?" → "`RequestData.param` 타입 때문" → "왜?" → "`encode()` 선언이 잘못됨" → `encode(obj: unknown)`으로 수정
2. 원인에서부터 해결한다. 오류가 연쇄적으로 발생하면 **오류가 처음 시작된 지점**을 찾아 거기서 수정한다
3. **회피책 금지**: 타입 에러를 `as`, `[key: string]: unknown`, `any` 추가로 "통과"시키지 않는다. 이 패턴은 오염을 코드베이스 전체로 전파시킨다
4. 근본 해결이 기술적으로 불가능한 경우에만 우회책을 **사용자에게 제안**한다:
