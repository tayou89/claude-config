...
- **Always characterize each option (multi-axis)**: When presenting decision options, label every option along these four axes: (1) **accuracy/correctness** (does it fully solve the problem?), (2) **safety/reversibility** (what happens if it fails — easy to undo or destructive?), (3) **standardness/textbook adherence** (canonical pattern vs. ad-hoc), (4) **effort/time** (cost). State explicitly which option is recommended and why, with the recommendation separated from each axis (an option can be most accurate but riskiest, or most standard but slowest). User should never have to ask "which is most accurate / safest / standard?" — every option in every prompt must show all four labels. Apply only when presenting 2+ distinct decision alternatives; for simple yes/no confirmations or single-action approvals, ask in plain prose without option labels (X1/X2/A1/B2 etc.).
