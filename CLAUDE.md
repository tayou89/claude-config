...
특정 파일에서 문제를 발견하면, **수정 전에 프로젝트 전체에서 같은 패턴의 문제가 있는지 전수 조사**한다. 해당 파일만 고치고 넘어가면 다음 파일에서 같은 문제가 반복된다.

예: `swapWorkIdx: number`가 실제로는 `string`(UUID)인 문제를 발견하면, 해당 파일만 고치지 말고 `grep -rn 'swapWorkIdx.*number'`로 전체를 검색해서 한꺼번에 수정한다.
