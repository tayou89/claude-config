...
## 빌드 절차: 스타일 검사 → 사용자 승인 → 빌드

`tsc` 빌드(산출물 생성)를 실행하기 **전에** 반드시 다음 절차를 따른다:

1. **스타일 검사**: 작성한 코드가 이 스킬의 스타일 규칙을 준수하는지 확인한다
2. **`npx tsc --noEmit`**: 타입 에러 0개를 확인한다 (산출물을 생성하지 않으므로 이 단계까지는 승인 없이 진행 가능)
3. **사용자 승인**: 코드를 사용자에게 보여주고 확인을 받는다
4. **`npx tsc` 빌드**: 승인 후에만 산출물(.js, .js.map)을 생성한다

**사용자 승인 없이 `npx tsc` (산출물 생성 빌드)를 실행하지 않는다.** 빌드 후 스타일 수정이 발생하면 재빌드가 필요하므로 승인을 먼저 받는다.
